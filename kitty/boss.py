...
class Boss:
    ...
    def _new_tab(
        self,
        args: SpecialWindowInstance | Iterable[str],
        cwd_from: CwdRequest | None = None,
    ...
    def _new_window(self, args: list[str], cwd_from: CwdRequest | None = None, persist: bool = False) -> Window | None:
        ...
        if args:
            w = tab.new_special_window(
                self.args_to_special_window(args, cwd_from=cwd_from), location=location, allow_remote_control=allow_remote_control, persist=persist
            )
        else:
