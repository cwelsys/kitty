...
class Tab:  # {{{
    ...
    def drag_resize_window(self, object_id: int, increment: float, is_horizontal: bool) -> float:
        if resized := self.current_layout.drag_resize_window(self.windows, object_id, increment, is_horizontal):
            self.relayout()
        return resized
...
class TabManager:  # {{{
    ...
    def new_tab(
        ...
        t = (
            ...
            if empty_tab
            else Tab(self, special_window=special_window, cwd_from=cwd_from, session_name=session_name, persist=persist)
        )
